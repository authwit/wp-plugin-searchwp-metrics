	...
	function __construct() {
		$this->strings = array(
			...
			'clear_metrics_data_before_note' => wp_kses(
				__( 'This will remove all the data Metrics has logged <strong>before</strong> the date specified below. Once cleared, this data <strong>cannot be restored!</strong>', 'searchwp-metrics' ),
				array(
					'strong' => array(),
				)
			),
		...
		$this->options = array(
			...
			'default_clear_data_before_date' => date( 'F j, Y', strtotime( apply_filters( 'searchwp_metrics_default_clear_data_before_date', '3 months ago' ) ) ),
