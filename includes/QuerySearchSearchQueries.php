	...
	function set_sql_where() {
		if ( class_exists( 'SearchWP_Stats' ) ) {
			...
			if ( ! empty( $ignored_queries ) ) {
				$this->sql['where'] .= " AND MD5(LOWER(`{$this->tables['queries']}`.`query`)) NOT IN ('" . implode( ', ', array_fill( 0, count( $ignored_queries ), '%s' ) ) . "')";
			}
			...
			if ( ! empty( $ignored_queries ) ) {
				$this->sql['where'] .= " AND LOWER(`{$this->tables['queries']}`.`query`) NOT IN ('" . implode( ', ', array_fill( 0, count( $ignored_queries ), '%s' ) ) . "')";
			}
		} else {
			return '';
		}
